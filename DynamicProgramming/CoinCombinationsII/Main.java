...
/**
 * <h1>Coin Combinations II</h1><br/>
 * Consider a money system consisting of {@code n} coins. Each coin has a positive integer value.
 * Task is to calculate the number of distinct ordered ways you can produce a money sum {@code x} using the available coins.<br/>
 * For example, if the coins are {@code {2,3,5}} and the desired sum is 9, there are 3 ways:
 * <ul>
 *     <li>{@code 2 + 2 + 5}</li>
 *     <li>{@code 3 + 3 + 3}</li>
 *     <li>{@code 2 + 2 + 2 + 3}</li>
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        final CoinCombinationsII algorithm = createAlgorithm();
        final int waysCount = algorithm.computeNumberOfWays();

        System.out.println(waysCount);
    }

    private static CoinCombinationsII createAlgorithm() throws IOException {
        try (Reader reader = new Reader()) {
            ...
            return new CoinCombinationsII(coins, minCoin, requiredSum);
        }
    }
    ...
    static class CoinCombinationsII {
        ...
        CoinCombinationsII(int[] coins, int minCoin, int requiredSum) {
        ...
        int computeNumberOfWays() {
            ...
            for (int coin : this.coins) {
                ...
                for (int i = coin; i <= this.requiredSum; i++) {
                    waysCount[i] = (waysCount[i] + waysCount[i - coin]) % MODULO;
                }
            }
