...
/**
 * <a href="https://cses.fi/problemset/task/3426/"><h1>Sliding Window Xor</h1><br/></a>
 * Given an array of {@code n} integers. Task is to calculate the bitwise xor of each window of {@code k} elements, from left to right.<br/>
 ...
 * </ul>
 * <i><b>Output</b></i>:<br/>
 * Print the xor of all window xors.<br/>
...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        final SlidingWindowXor algorithm = readInputData();
        final long result = algorithm.execute();

        System.out.println(result);
    }

    private static SlidingWindowXor readInputData() throws Exception {
        try (Reader reader = new Reader()) {
            ...
            return new SlidingWindowXor(n, k, x, a, b, c);
        }
    }

    static class SlidingWindowXor {
        ...
        SlidingWindowXor(int n, int k, int x, int a, int b, int c) {
            this.n = n;
            this.k = k;
            this.x = x;
            this.a = a;
        ...
        long execute() {
            ...
            final long[] xors = new long[this.n - this.k + 1];
            ...
            for (int i = this.k; i < this.n; i++) {
                numbers[i] = currentX;
                xors[i - this.k + 1] = (numbers[i - this.k] ^ xors[i - this.k]) ^ currentX;
