 ...
 * number (odd will not be divisible by {@literal 2}). Accordingly, first need to find the sum of a series of numbers
 * from {@literal 1} to {@literal n}. To do this, we use the arithmetic progression formula with step {@literal 1}.
 * Next, we divide the sum by {@literal 2} and find the sum that each of the two sets should make up.
 ...
 * sum and the resulting number, if it is not equal to {@literal 0}, are the numbers that make up the first set.
